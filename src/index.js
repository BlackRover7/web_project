  ...
  return (
    <div className="about">
      <ul id="listAbout">
        ...
        <li><p>Выполнил студент группы РПИС-92, Ушаков Сергей</p></li>
      </ul>
    </div>
...
export function PageNotFound() {
  return (
    <div className='errorpage'>
      <h1>:(</h1>
      <h1>404</h1>
      <h2>Упс, такой странички нет, вернитесь на <p><Link to={"/"}>главную страничку</Link></p></h2>
    </div>
  );
}
      ...
      <div>
        <div>
          <div className="searchSmth">
            <input id='inputTitle' type="text" placeholder='Поиск по заголовку:' />
            <button className="sortBtnText" onClick={() => handleOnClickText("title_contains=" + document.getElementById('inputTitle').value)}> Поиск</button>
          </div>
          <div className="searchSmth">
            <input id='inputSumm' type="text" placeholder='Поиск по содержанию:' />
            <button className="sortBtnText" onClick={() => handleOnClickText("summary_contains=" + document.getElementById('inputSumm').value)}> Поиск</button>
          </div>
          <div id='sortbuttons'>
  ...
  if (error) {
    ...
    return (
      <div>
        <ul className="newsList">
          <li key={item.id}>
            <p id="newsTitle">{item.title} {!isBookmarked ?
              <button className="bookmarkBtn" onClick={() => handleClickSave(item.id)}>  В закладки</button> :
              <button className="bookmarkBtn" onClick={() => handleClickRemove(item.id)}> Убрать из закладок</button>
            }</p>
        ...
        <hr />
        <p />
        <label id="sameNewsLbl">Похожие статьи:</label>
        <p />
        <hr />
        <div>
          <ul className="newsList">
